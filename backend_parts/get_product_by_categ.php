<?php
    ...
    function getByCateg($categName){

        $sanitized = htmlspecialchars($categName);
        if(!isset($conn)){
            include_once('con_db.php');
        }

        $stmt = $conn->prepare("SELECT * FROM product_list WHERE prod_categ=?");
        $stmt->bind_param("s",$sanitized);
        $stmt->execute();
